...
- source gating now applies query-family cleanup to both display sources and reasoning sources before LLM generation: frontend UI-location questions keep frontend sources, indexing/retrieval explanations exclude unrelated frontend/eval/report/benchmark artifacts, and broad overview prompts suppress meta helper sources in the context passed to the LLM
...
- **Response Quality and Source Relevance V2 is complete** — `query_intent.py` defines source-intent contracts for overview, runtime architecture, frontend/backend flow, repository analysis, indexing, incremental indexing, retrieval/source filtering, UI implementation, API endpoints, provider configuration, failure recovery, and indexing freshness/status. The source contract is now generic: it does not hardcode CodeSeek paths. `searcher.py` discovers repair candidates from the active repo checkout by scoring README/docs, manifests/config, API routes/controllers/servers, frontend components/hooks/API clients, indexing/ingestion modules, retrieval/search/RAG modules, provider/settings files, job/status/database/error-handling files, and troubleshooting docs. `source_filter.py` uses the same generic contract to promote preferred source roles and demote helper, scratch, test, benchmark, and irrelevant provider endpoint sources. Deterministic/LLM answers are post-processed to remove manual `Sources:` sections, failure-recovery speculation is rejected, and focused validation passed with `22` backend tests plus the frontend source-card/diagnostics tests and frontend build.
