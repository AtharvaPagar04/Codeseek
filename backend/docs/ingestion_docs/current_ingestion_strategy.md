...
12. embed chunks
13. upsert points into Qdrant
14. optionally delete removed-file chunks when incremental mode is enabled
15. persist next ingestion state
16. print a terminal report
...
This stage is currently the main reason repo-level answers are weaker than symbol-level answers.
...
are usually not ingested at all, including:
...
The retrieval layer contains heuristics that try to prioritize some of these files, but ingestion often never puts them into the vector store in the first place.
...
`rag_ingestion/stages/parser.py` parses supported files with Tree-sitter.

### 10.1 Parser selection

Parser selection is extension-based:
...
These summaries are useful for metadata retrieval and lexical overlap, but they are shallow. They do not describe:
...
Raw `content` is not stored in payload. Retrieval reconstructs content later by reopening the repo file and slicing lines by stored line numbers.
...
### 19.1 Repo-level files are mostly invisible

This is the highest-impact weakness.

The current pipeline is optimized for source code and ignores many files that matter for:
...
### 19.4 Summaries are thin

Current deterministic summaries help with symbol lookup, but they do not create high-quality semantic descriptions for:
...
### 20.1 Highest priority: ingest non-code evidence
